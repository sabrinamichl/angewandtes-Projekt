...
raw4a %>% 
  filter(BZG < 0.5) 
```
 > 30 Personen sind weniger als 1 Jahr bei ihrem aktuellen Unternehmen beschäftigt
...
```{r}
library(CTT)
spearman.brown(0.7580887, input = 13/7 ,n.or.r = "n")
```
...
### QQ-Anand
```{r}
lm1 <- lm(QQ_Anand_mean ~ OC_A_mean, data=d_work_s)
summary(lm1)
```

```{r}
d_work %>% 
  filter(is.na(Gehalt_n & Alter))
```
...
### Gesamte QQ-Skala
```{r}
lm2 <- lm(QQ_Gesamt_mean ~ OC_A_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm2)
```
...
### QQ-Anand
```{r}
lm3 <- lm(QQ_Anand_mean ~ IS_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm3)
```

### Gesamte QQ-Skala
```{r}
lm4 <- lm(QQ_Gesamt_mean ~ IS_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm4)
```
...
### QQ-Anand
```{r}
lm5 <- lm(QQ_Anand_mean ~ CPE_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm5)
```

### Gesamte QQ-Skala
```{r}
lm6 <- lm(QQ_Gesamt_mean ~ CPE_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm6)
```
...
### QQ-Anand
```{r}
lm7 <- lm(QQ_Anand_mean ~ PS_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm7)
```

### Gesamte QQ-Skala
```{r}
lm8 <- lm(QQ_Gesamt_mean ~ PS_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm8)
```
...
### QQ-Anand
```{r}
lm9 <- lm(QQ_Anand_mean ~ SL_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm9)
```

### Gesamte QQ-Skala
```{r}
lm10 <- lm(QQ_Gesamt_mean ~ SL_mean + Alter + Geschlecht + Gehalt_n, data=d_work_s)
summary(lm10)
```
...
## QQ_Anand
```{r}
lm11 <- lm(QQ_Anand_mean ~ OC_A_mean*CPE_mean, data=d_work_s)
summary(lm11)
```
...
## QQ_Gesamt
```{r}
lm12 <- lm(QQ_Gesamt_mean ~ OC_A_mean*CPE_mean, data=d_work_s)
summary(lm12)
```

```{r}
interact_plot(model=lm12, pred=OC_A_mean, modx=CPE_mean)
```
...
## QQ_Anand
```{r}
lm13 <- lm(QQ_Anand_mean ~ OC_A_mean*Alter_kat, data=d_work_plot_lm_1)
summary(lm13)
```

```{r}
interact_plot(model=lm13, pred=OC_A_mean, modx=Alter_kat)
```

## QQ_Gesamt
```{r}
lm14 <- lm(QQ_Gesamt_mean ~ OC_A_mean*Alter_kat, data=d_work_plot_lm_1)
summary(lm14)
```

```{r}
interact_plot(model=lm14, pred=OC_A_mean, modx=Alter_kat)
```

# Moderation: CPE moderiert den Zusammenhang von SL und QQ
```{r}
ggplot(d_work_plot_lm)+
  geom_jitter(mapping = aes(x=SL_mean, y=QQ_Gesamt_mean, color=as.factor(CPE)))+
  geom_smooth(mapping = aes(x=SL_mean, y=QQ_Gesamt_mean, color=as.factor(CPE)), method = "lm", se=FALSE)
```

## QQ_Anand
```{r}
lm15 <- lm(QQ_Anand_mean ~ SL_mean*CPE_mean, data=d_work_s)
summary(lm15)
```

```{r}
interact_plot(model=lm15, pred=SL_mean, modx=CPE_mean)
```


## QQ_Gesamt
```{r}
lm16 <- lm(QQ_Gesamt_mean ~ SL_mean*CPE_mean, data=d_work_s)
summary(lm16)
```

```{r}
interact_plot(model=lm16, pred=SL_mean, modx=CPE_mean)
```
