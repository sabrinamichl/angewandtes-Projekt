...
#### QQ_Anand
```{r}
lm11 <- lm(QQ_Anand_mean ~ OC_A_mean + OC_A_mean*CPE_mean + Alter + Geschlecht + BZG + Gehalt, data=d_work_s)
summary(lm11)
```
#### QQ_Gesamt
```{r}
lm12 <- lm(QQ_Gesamt_mean ~ OC_A_mean + OC_A_mean*CPE_mean + Alter + Geschlecht + BZG + Gehalt, data=d_work_s)
summary(lm12)
```
...
### Hypothesenprüfung
```{r}
lm13 <- lm(QQ_Anand_mean ~ OC_A_mean + OC_A_mean*Alter_kat + Alter + Geschlecht + BZG + Gehalt, data=d_work_plot_lm_1)
summary(lm13)
```

```{r}
lm14 <- lm(QQ_Gesamt_mean ~ OC_A_mean + OC_A_mean*Alter_kat + Alter + Geschlecht + BZG + Gehalt, data=d_work_plot_lm_1)
summary(lm14)
```
