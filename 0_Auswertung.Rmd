...
# `BZG_J` und `BZG_M` zusammenrechnen
```{r}
raw4a <- raw4 %>% 
...
# Verteilungen der Soziodemografika
## Geschlecht
```{r}
raw4a %>% 
  count(Geschlecht) %>% 
...
# Kurzen Datensatz `raw` abspeichern
```{r}
...
# Kurzen Datensatz `d_work` abspeichern
```{r}
...
subset_cor <- subset(d_work,
                     select = c(QQ_01, QQ_02, QQ_03, QQ_04, QQ_05, QQ_06, QQ_07, QQ_N08, QQ_N09, QQ_N10, QQ_N11, QQ_N12, QQ_N13, QQ_N14))
```

```{r}
library(apaTables)
apa.cor.table(subset_cor, filename = "corrrrr.doc", table.number = 2)
```
...
```{r}
d_work_s1 <- d_work_s %>% 
  select(ID, IS_mean, IS_K_mean, IS_E_mean, CPE_mean, CPE_C_mean, CPE_I_mean, CPE_M_mean, CPE_E_mean, PS_mean, OC_A_mean, QQ_mean, SL_mean)
```
...
```{r}
subset_cor <- subset(d_work,
                     select = c(ID, IS_mean, IS_K_mean, IS_E_mean, CPE_mean, CPE_C_mean, CPE_I_mean, CPE_M_mean, CPE_E_mean, PS_mean, OC_A_mean, QQ_Anhand_mean, QQ_Eigene_mean, QQ_Gesamt_mean, SL_mean))
```
