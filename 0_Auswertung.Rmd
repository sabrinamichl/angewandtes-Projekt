...
## Daten einlesen
```{r}
d_raw <- read_excel("Pre-Test-Test.xlsx")
```

## Unnötige Variablen entfernen
```{r}
raw1 <- d_raw %>% 
  select(-c(CASE, SERIAL, REF, QUESTNNR, MODE, STARTED, TIME001:TIME012, LASTDATA, FINISHED, Q_VIEWER, LASTPAGE, MAXPAGE, MISSREL, MISSING, MAILSENT))
```
...
```{r}
psych::alpha(x=subset(d_work, select=c(QQ_01:QQ_07)), check.keys=TRUE)
```


```{r}
subset_cor <- subset(d_work,
                     select = c(QQ_01, QQ_02, QQ_03, QQ_04, QQ_05, QQ_06, QQ_07))
```
...
```{r}
subset_cor <- subset(d_work,
                     select = c(ID, IS_mean, IS_K_mean, IS_E_mean, CPE_mean, CPE_C_mean, CPE_I_mean, CPE_M_mean, CPE_E_mean, PS_mean, OC_A_mean, QQ_mean, SL_mean))
```
