...
## Recodieren der eigenen QQ Skala 
```{r}
raw4 <- raw4 %>% 
  mutate(across(
    .cols = c(QQ_N10, QQ_N11),
    .fns = ~dplyr::recode(.,
                    `1` = 5,
                    `2` = 4,
                    `3` = 3,
...
```{r}
raw4$CPE_M11[61] <- 4.675393
raw4$SL_05[61] <-3.445026
```
...
raw4a %>% 
  ...
  geom_vline(xintercept=38.95263 , color="thistle2", size=2)+
  geom_vline(xintercept=52.48225, color="turquoise1", size=1)+
  geom_vline(xintercept=25.42301, color="turquoise1", size=1)+
  theme_minimal()
```
...
```{r}
raw4a %>% 
  summarise(mean(Arbeitszeitmodell, na.rm=TRUE),
            median(Arbeitszeitmodell, na.rm=TRUE),
         sd(Arbeitszeitmodell, na.rm=TRUE),
         min(Arbeitszeitmodell, na.rm=TRUE),
         max(Arbeitszeitmodell, na.rm=TRUE))
```
...
#### Hypothesenprüfung
```{r}
lm1 <- lm(QQ_Gesamt_mean ~ OC_A_mean+ OCA_z*CPE_z, d_work_s_TEST)
summary(lm1)
```
