...
## QQ_Anand
```{r}
lm13 <- lm(QQ_Anand_mean ~ OC_A_mean*Alter, data=d_work_s)
summary(lm13)
```

```{r}
interact_plot(model=lm13, pred=OC_A_mean, modx=Alter)
```

## QQ_Gesamt
```{r}
lm14 <- lm(QQ_Gesamt_mean ~ OC_A_mean*Alter, data=d_work_s)
summary(lm14)
```

```{r}
interact_plot(model=lm14, pred=OC_A_mean, modx=Alter)
```
