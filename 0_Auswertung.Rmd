...
raw4_6 %>% 
  ...
  aes(x=Alter)+
  geom_histogram(fill = "green4")+
  geom_vline(xintercept=40.26437 , color="orange2", size=2)+
  geom_vline(xintercept=53.6445, color="yellow3", size=1)+
  geom_vline(xintercept=26.88424, color="yellow3", size=1)+
  theme_minimal()
```
...
```{r}
write.csv(raw4_8, file="raw4.csv")
```
...
## Irritation
```{r}
psych::alpha(subset(raw4_8, select=c(IS_K01:IS_E08)), check.keys=TRUE)
```

## CPE
```{r}
psych::alpha(subset(raw4_8, select=c(CPE_C01:CPE_E16)), check.keys=TRUE)
```

## PS
```{r}
psych::alpha(subset(raw4_8, select=c(PS_01:PS_07)), check.keys=TRUE)
```

## OCA
```{r}
psych::alpha(subset(raw4_8, select=c(OC_A01:OC_A05)), check.keys=TRUE)
```

## QQ
### QQ-Anand
```{r}
psych::alpha(subset(raw4_8, select=c(QQ_01:QQ_07), check.keys=TRUE))
```

### QQ-Neu
```{r}
psych::alpha(subset(raw4_8, select=c(QQ_N08:QQ_N14), check.keys=TRUE))
```

### QQ-Gesamt
```{r}
psych::alpha(subset(raw4_8, select=c(QQ_01:QQ_N14), check.keys=TRUE))
```
...
```{r}
library(CTT)
spearman.brown(0.7522749, input = 13/7 ,n.or.r = "n")
```
  > Aufgrund dessen, dass eine Verlängerung der Anand-Skala eine Reliabilität von 0.8493896 vorweisen würde und diese die Relibilität der von uns verlängerten Skala 0.8540333 beträgt (-0.0046437). Da sich hierdurch nur eine geringe Verbesserung zeigt und dies unverhältnismäßig zur Komplexität der Skala beitragen würde, wird im Folgenden mit der Originalskala gerechnet.
...
```{r}
lm1 <- lm(QQ_Anand_mean ~ CPE_mean, data=d_work_lm)
summary(lm1)
```
...
```{r}
effectsize::interpret_r2(0.3766, rules="cohen1988")
```


## H2: Organisationales Commitment - affektiv `OCA` 
```{r}
lm2 <- lm(QQ_Anand_mean ~ OC_A_mean, data=d_work_lm)
summary(lm2)
```
...
### Effektgröße beschreiben
```{r}
effectsize::interpret_r2(0.4178, rules="cohen1988")
```
...
```{r}
lm3 <- lm(QQ_Anand_mean ~ PS_mean, data=d_work_lm)
summary(lm3)
```
...
### Effektgröße beschreiben
```{r}
effectsize::interpret_r2(0.219, rules="cohen1988")
```
...
## H4: Irritation `IS`
```{r}
lm4 <- lm(QQ_Anand_mean ~ IS_mean, data=d_work_lm)
summary(lm4)
```
...
### Effektgröße beschreiben
```{r}
effectsize::interpret_r2(0.04226, rules="cohen1988")
```
...
```{r}
lm5 <- lm(QQ_Anand_mean ~ SL_mean, data=d_work_lm)
summary(lm5)
```
...
### Effektgröße beschreiben
```{r}
effectsize::interpret_r2(0.3059, rules="cohen1988")
```
...
ggplot(d_work_plot_lm)+
  geom_jitter(mapping = aes(x=OC_A_mean, y=QQ_Anand_mean))+
  geom_smooth(mapping = aes(x=OC_A_mean, y=QQ_Anand_mean, color=as.factor(CPE)), method = "lm", se=FALSE)+
  theme_minimal()
```


```{r}
lm6 <- lm(QQ_Anand_mean ~ OC_A_mean*CPE_mean, data=d_work_lm)
summary(lm6)
```
...
plot_lm6 <-interact_plot(model=lm6, pred=OC_A_mean, modx=CPE_mean)
plot_lm6
```
...
### QuantPsyc
  > Modell zentriert direkt die Prädiktoren 

```{r}
library(QuantPsyc)
fitMod_lm6 <- moderate.lm(x=OC_A_mean, z=CPE_mean, y=QQ_Anand_mean, data=d_work_lm,
                      mc=FALSE)
summary(fitMod_lm6)
```
...
```{r}
simpleSlope_lm6 <- sim.slopes(mod=fitMod_lm6, z=d_work_lm$CPE_mean, zsd=1,
                           mcz=FALSE)
```

```{r}
graph.mod(simpleSlope_lm6, x=OC_A_mean, y=QQ_Anand_mean, data=d_work_lm, ylab="AV", xlab="UV")
```
...
## QQ_Anand
```{r}
lm7 <- lm(QQ_Anand_mean ~ OC_A_mean*Alter_kat, data=d_work_lm)
summary(lm7)
```
```{r}
confint(lm7)
...
```{r}
plot_lm7 <-interact_plot(model=lm7, pred=OC_A_mean, modx=Alter_kat,
              interval = TRUE)
plot_lm7
```
