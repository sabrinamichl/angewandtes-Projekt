...
# Bearbeitungszeit genauer betrachten
```{r}
raw4a %>% 
  select(TIME_SUM) %>% 
  mutate(TIME_SUM = TIME_SUM/60) %>% 
  summarise(mean(TIME_SUM),
            sd(TIME_SUM))
```
 >> Die Probanden benötigen im Mittel 9 Minuten zum Ausfüllen mit einer Standardabweichung von 2.65 Minuten
...
raw4a %>% 
  ...
  geom_vline(xintercept=36.25 , color="thistle2", size=2)+
  geom_vline(xintercept=49,60016, color="turquoise1", size=1)+
  geom_vline(xintercept=22,99984, color="turquoise1", size=1)+
  theme_minimal()
```
...
```{r}
raw4a %>% 
  count(Arbeitszeitmodell) %>% 
  mutate(mean(Arbeitszeitmodell, na.rm=TRUE),
         sd(Arbeitszeitmodell, na.rm=TRUE),
         prob = n/sum(n)) %>% 
  round(2)
```
...
```{r}
write.csv(raw4, file="raw4.csv")
```

# 8. Skalen zusammenfassen
## Irriation (IS)
```{r}
d_work <- raw4a
d_work$IS_mean <- rowMeans(subset(raw4a, select = c(IS_K01, IS_K02, IS_K03, IS_K04, IS_E05, IS_E06, IS_E07, IS_E08)))
d_work$IS_K_mean <- rowMeans(subset(raw4a, select = c(IS_K01, IS_K02, IS_K03, IS_K04)))
d_work$IS_E_mean <- rowMeans(subset(raw4a, select = c(IS_E05, IS_E06, IS_E07, IS_E08)))
```
...
d_work$CPE_C_mean <- rowMeans(subset(raw4a, select = c(CPE_C01, CPE_C02, CPE_C03, CPE_C04)))
d_work$CPE_I_mean <- rowMeans(subset(raw4a, select = c(CPE_I05, CPE_I06, CPE_I07, CPE_I08)))
d_work$CPE_M_mean <- rowMeans(subset(raw4a, select = c(CPE_M09, CPE_M10, CPE_M11, CPE_M12)))
d_work$CPE_E_mean <- rowMeans(subset(raw4a, select = c(CPE_E13, CPE_E14, CPE_E15, CPE_E16)))
```

## Psychologische Sicherheit (PS)
```{r}
d_work$PS_mean <- rowMeans(subset(raw4a, select = c(PS_01, PS_02, PS_03, PS_04, PS_05, PS_06, PS_07)))
```

## Organisationelles Commitment (OCA)
```{r}
d_work$OC_A_mean <- rowMeans(subset(raw4a, select = c(OC_A01, OC_A02, OC_A03, OC_A04, OC_A05)))
```

## Quiet Quitting
```{r}
d_work$QQ_Anand_mean <- rowMeans(subset(raw4a, select = c(QQ_01, QQ_02, QQ_03, QQ_04, QQ_05, QQ_06, QQ_07)))
d_work$QQ_Eigene_mean <- rowMeans(subset(raw4a, select = c(QQ_N08, QQ_N09, QQ_N10, QQ_N11, QQ_N12, QQ_N13, QQ_N14)))
d_work$QQ_Gesamt_mean <- rowMeans(subset(raw4a, select = c(QQ_01, QQ_02, QQ_03, QQ_04, QQ_05, QQ_06, QQ_07, QQ_N08, QQ_N09, QQ_N10, QQ_N11, QQ_N12, QQ_N13, QQ_N14)))
```

## Servant Leadership
```{r}
d_work$SL_mean <- rowMeans(subset(raw4a, select = c(SL_01, SL_02, SL_03, SL_04, SL_05, SL_06, SL_07)))
```

## Einzelne Items aus dem Datensatz entfernen
```{r}
d_work_s <- d_work %>% 
  select(-c(IS_K01:IS_E08, OC_A01:OC_A05, CPE_C01:CPE_E16, PS_01:PS_07, QQ_01:QQ_07, QQ_N08:QQ_N14, SL_01:SL_07, TIME_SUM, TIME_RSI)) %>% 
  select(ID, everything())
```

# 9. Kurzen Datensatz `d_work` abspeichern
```{r}
write.csv(d_work_s, file="d_work_s.csv")
```

# 10. Cronbachs Alpha berechnen
## Irritation
```{r}
...
### QQ-Neu
```{r}
psych::alpha(subset(d_work, select=c(QQ_N08:QQ_N14), check.keys=TRUE))
```

### QQ-Gesamt
```{r}
psych::alpha(subset(d_work, select=c(QQ_01:QQ_N14), check.keys=TRUE))
```
...
## PE moderiert den Zusammenhang von OCA und QQ-Gesamt
```{r}
...
d_work_s_TEST <- d_work_s %>% 
  mutate(CPE_z = (CPE_mean - mean(CPE_mean))/sd(CPE_mean),
         OCA_z = (OC_A_mean - mean(OC_A_mean))/sd(OC_A_mean),
         QQ_gesamt_z = (QQ_Gesamt_mean - mean(QQ_Gesamt_mean))/sd(QQ_Gesamt_mean))
```

#### Hypothesenprüfung
```{r}
lm1 <- lm(QQ_Gesamt_mean ~ OCA_z + OCA_z*CPE_z, d_work_s_TEST)
summary(lm1)
```



## Geschlecht moderiert den Zusammenhang von OCA und QQ-Gesamt
```{r}
ggplot(d_work_plot_lm)+
  geom_jitter(mapping = aes(x=OC_A_mean, y=QQ_Gesamt_mean, color=as.factor(Geschlecht)))+
  geom_smooth(mapping = aes(x=OC_A_mean, y=QQ_Gesamt_mean, color=as.factor(Geschlecht)), method = "lm", se=FALSE)
```

### Hypothesenprüfung
```{r}
lm2 <- lm(QQ_Gesamt_mean ~ OC_A_mean, d_work_s)
summary(lm2)
```
