...
```{r}
d_work_lm %>%
  summarise(cor_QQ_CPE = cor(CPE_mean, QQ_Anand_mean, use="complete.obs"))
```
...
```{r}
d_work_lm %>%
  summarise(cor_QQ_OCA = cor(OC_A_mean, QQ_Anand_mean, use="complete.obs"))
```
...
```{r}
d_work_lm %>%
  summarise(cor_QQ_PS = cor(PS_mean, QQ_Anand_mean, use="complete.obs"))
```
...
```{r}
d_work_lm %>%
  summarise(cor_QQ_IS = cor(IS_mean, QQ_Anand_mean, use="complete.obs"))
```
...
```{r}
d_work_lm %>%
  summarise(cor_QQ_SL = cor(SL_mean, QQ_Anand_mean, use="complete.obs"))
```
...
```{r}
lm06 <- lm(QQ_Anand_mean ~ OC_A_mean*CPE_mean, data=d_work_lm)
summary(lm06)
```
...
plot_lm06 <-interact_plot(model=lm6, pred=OC_A_mean, modx=CPE_mean)
plot_lm06
```
